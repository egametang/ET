...
{
    public class UICreateCommonSystemCode : BaseTemplate
    {
        ...
        public UICreateCommonSystemCode(out bool result, string authorName, UICreateSystemData codeData) : base(authorName)
        {
            var path     = $"{UIStaticHelper.UIETSystemPath}/{codeData.PkgName}/{codeData.ResName}ComponentSystem.cs";
            var template = $"{UIStaticHelper.UITemplatePath}/UICreateCommonSystemTemplate.txt";
