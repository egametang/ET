...
namespace ET
{
    public class CodeLoader : IDisposable
    {
        ...
        public async void Start()
        {
            ...
            await YooAssets.InitializeAsync(initParameters).Task;
            switch (this.CodeMode)
            {
                case CodeMode.Mono:
                    {
                        byte[] assBytes = await YooAssetManager.Instance.LoadRawFileBytesAsync("Code.dll");
                        byte[] pdbBytes = await YooAssetManager.Instance.LoadRawFileBytesAsync("Code.pdb");
                ...
                case CodeMode.HybridCLR:
                    {
                        TextAsset codeDll = await YooAssetManager.Instance.LoadByAddressAsync<TextAsset>("Assets/Bundles/Code/Code.dll.bytes");
                        TextAsset codePdb = await YooAssetManager.Instance.LoadByAddressAsync<TextAsset>("Assets/Bundles/Code/Code.dll.bytes");
